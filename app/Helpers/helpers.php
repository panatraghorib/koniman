...
if (!function_exists('parse_value')) {
    function parse_value($value, $type = 'raw')
    {
        if ($value != '' && \Illuminate\Support\Str::endsWith(strtolower($value), ['png', 'jpg', 'jpeg', 'gif', 'svg'])) {
...
if (!function_exists('encode_id')) {
     ...
     * Prepare the Column Name for Lables.
     */
    function encode_id($id, $num = 100)
    {
        $hashids = new Hashids\Hashids(config('app.salt'), $num, 'abcdefghijklmnopqrstuvwxyz1234567890^.@');
...
if (!function_exists('decode_id')) {
     ...
     * Prepare the Column Name for Lables.
     */
    function decode_id($hashid, $num = 100)
    {
        $hashids = new Hashids\Hashids(config('app.salt'), $num, 'abcdefghijklmnopqrstuvwxyz1234567890^.@');
