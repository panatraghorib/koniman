    ...
    <transition name="alert">
        ...
        <div
            ...
            :class="[
                ...
                !type
                    ? `alert-${temporaryDesign}-${temporaryColor}`
                    : [
...
/*Sources*/
import { alertConf } from "@/tailconfig.js";
...
export default defineComponent({
    ...
    setup(props, { slots, emit }) {
        ...
        const temporaryDesign = ref(
            design.value
                ? design.value
                : alertConf.design
                ? alertConf.design
                : appConf.value.design
        );
        const temporaryColor = ref(
            color.value
                ? color.value
                : alertConf.color
                ? alertConf.color
        ...
        const temporaryRadius = ref(
            radius.value
                ? radius.value
                : alertConf.radius
                ? alertConf.radius
                : appConf.value.radius
        );
        const temporaryCloseable = ref(
            closeable.value
                ? closeable.value
                : alertConf.closeable
                ? alertConf.closeable
                : appConf.value.closeable
        );
        const temporaryTimer = ref(
            timer.value
                ? timer.value
                : alertConf.timer
                ? alertConf.timer
                ...
                let countDownFn = setInterval(() => {
                    if (temporaryTimer.value >= timerCounter.value) {
                        countDownCounter.value =
                            100 -
                            (timerCounter.value / temporaryTimer.value) * 100;
