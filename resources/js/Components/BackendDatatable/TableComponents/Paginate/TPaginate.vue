    ...
    >
        ...
        <div v-if="jump" class="pagination-detail-jump-container">
            <input
                ...
                :style="{
                    width:
                        Math.floor(total.toString().length) -
                        (0.25 * total.toString().length - 0.5) +
                        'rem',
                }"
            />
            <span
                @click="selectPage(Number(jumpPage))"
                class="pagination-detail-jump-button"
            >
                <svg
        ...
        <!--Counter Container-->
        <div class="pagination-counters" :class="`radius-${radius}`">
            <!--Previous-->
            <div
                :class="
                    range >= modelValue
            ...
            >
                <svg
                    ...
                    :class="[
                        'h-6 w-6',
                        arrowText ? '-mx-1 md:-ml-2 md:-mr-1' : '',
                    ]"
                    fill="none"
                    viewBox="0 0 24 24"
                    stroke="currentColor"
            ...
            <div
                :class="
                    Math.ceil(modelValue / range) ===
                    Math.ceil(totalPage / range)
                        ? 'pagination-passive-arrow'
                        : 'pagination-arrow'
                "
                @click="nextPage"
            >
                ...
                <svg
                    ...
                    :class="[
                        'h-6 w-6',
                        arrowText ? '-mx-1 md:-mr-2 md:-ml-1' : '',
                    ]"
                    fill="none"
                    viewBox="0 0 24 24"
                    stroke="currentColor"
...
import pagination_tr from "@/Lang/tr/pagination_lang.js";
import pagination_en from "@/Lang/en/pagination_lang.js";
...
export default defineComponent({
    ...
    setup(props, { emit }) {
        ...
        const nextPage = () => {
            if (
                Math.ceil(totalPage.value / range.value) >
                Math.ceil(modelValue.value / range.value)
            ) {
                let newPage =
                    Math.ceil(modelValue.value / range.value) * range.value + 1;
        ...
        const previousPage = () => {
            if (modelValue.value > range.value) {
                let newPage =
                    Math.floor(modelValue.value / range.value) * range.value;
