<template>
    <transition name="modal">
        ...
        <div v-if="modelValue" :class="tStyle['container']">
            ...
            <!--Modal Content Box-->
            <div :class="tStyle['box']" ref="modalItem">
                <!--Header-->
                <div class="modal-header">
                    <!--Titles-->
                    <div
                        v-if="design === 'elegant' && (title || subTitle)"
                        id="title-container"
                    >
                        <span
                            v-if="design === 'elegant'"
                ...
                <div :class="tStyle['content']">
                    ...
                    <div v-if="icon || hasSlot('icon')" class="modal-icon">
                        <svg
                            v-if="icon"
                            v-html="
                                iconList.find((i) => i.key === icon).content
                            "
                            :class="tStyle['icon']"
                        />
                    ...
                    >
                        <span
                            v-if="design !== 'elegant' && title"
                            id="content-title"
                            v-text="title"
                        />
                        <span
                            v-if="design !== 'elegant' && subTitle"
                    ...
                    <!--Content-->
                    <div
                        v-if="content || hasSlot('content')"
                        class="modal-content"
                    >
                        <span
                            v-if="content"
                            v-html="content"
                            class="modal-content-default"
                        />
                        <slot name="content" />
                    </div>
                </div>
...
export default defineComponent({
    ...
    setup(props, { slots, emit }) {
            ...
            const countDownFn = setInterval(() => {
                if (timer.value >= timerCounter.value) {
                    countDownCounter.value =
                        (timerCounter.value / timer.value) * 100;
        ...
        tStyle["content"] = computed(() => {
            let style;
            if (
                (content.value || title.value || subTitle.value) &&
                !hasSlot("content")
            ) {
