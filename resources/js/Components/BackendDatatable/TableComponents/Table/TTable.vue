<template>
    ...
    >
        ...
        <transition name="tableSearch">
            <div v-if="hasSlot('search') && showAdvancedSearchPanel">
                <div
            ...
            <table class="table-container">
                ...
                <tbody>
                    ...
                    <tr
                        ...
                        :class="[
                            {
                                'table-content-zebra-row':
                                    features.table.zebraRow,
                            },
                            {
                                'table-content-hover-row':
                                    features.table.hoverRow,
                            },
                            {
                                'table-content-border-row':
                                    features.table.borderRow,
                            },
                        ]"
                    >
                        ...
                        >
                            ...
                            >
                                ...
                                <!--SlotScope Content-->
                                <slot v-else :name="cellKey" :props="item" />
                            </div>
                        </td>
                    </tr>
            ...
            <template #content>
                <div class="flex flex-col text-left">
                    <h4
                        ...
                        v-text="optionsModalColumnOrder"
                    />
    ...
    setup(props, { slots }) {
        ...
        const contentCellStyle = (itemIndex, cellIndex) => {
            ...
            style =
                "table-content-cell" +
                " " +
                (itemIndex === 0 && cellIndex === 0
                    ? "radius-tl-" + features.value["table"].radius
                    : itemIndex === 0 &&
                      cellIndex === regeneratedHeader.value.length - 1
                    ? "radius-tr-" + features.value["table"].radius
                    : itemIndex + 1 === diffOperator && cellIndex === 0
                    ? "radius-bl-" + features.value["table"].radius
                    : itemIndex + 1 === diffOperator &&
                      cellIndex === regeneratedHeader.value.length - 1
                    ? "radius-br-" + features.value["table"].radius
                    : "");
            return style;
        };
        ...
        const paginatedContent = computed(() => {
            return sortedContent.value.slice(
                (activePage.value - 1) * perPageItem.value,
                perPageItem.value * activePage.value
            );
        });

        const filteredContent = computed(() => {
            if (
                header.value.filter((h) => h.simpleSearchable === true).length >
                0
            ) {
                return content.value.filter((item) => {
                    let query;
                    header.value
                        .filter((h) => h.simpleSearchable === true)
                        .forEach((i) => {
                            query =
                                item[i.key]
                                    .toLowerCase()
